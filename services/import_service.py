...
def process_commerzbank(transactions_df: pd.DataFrame, exchange_rates_df: pd.DataFrame) -> pd.DataFrame:
...
def process_bank_file(bank: str, transactions_file, exchange_rates_file, field_mappings: Dict[str, Dict]) -> pd.DataFrame:
    ...
    if bank_config.get('needs_currency_conversion', False):
        ...
        transactions_df = process_currency_conversion(transactions_df, exchange_rates_df, bank_config['from_currency'], bank_config['to_currency'])
    
    return transactions_df

def process_currency_conversion(transactions_df: pd.DataFrame, exchange_rates_df: pd.DataFrame, from_currency: str, to_currency: str) -> pd.DataFrame:
    # Ensure date columns are in datetime format
    transactions_df['transaction_date'] = pd.to_datetime(transactions_df['transaction_date'])
    exchange_rates_df['Date'] = pd.to_datetime(exchange_rates_df['Date'])
    
    # Merge transactions with exchange rates
    df = transactions_df.merge(exchange_rates_df[['Date', 'Close']], left_on='transaction_date', right_on='Date', how='left')
