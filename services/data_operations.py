...
def add_account(name, account_type, institution, bank_identifier):
    session = get_session()
    new_account = Account(
        name=name, type=account_type, institution=institution, bank_identifier=bank_identifier
    )
    ...
    try:
        ...
        raise AccountAlreadyExistsError(
            f"An account with the name '{name}' or bank identifier '{bank_identifier}' already exists."
        )
    except SQLAlchemyError as e:
        session.rollback()
