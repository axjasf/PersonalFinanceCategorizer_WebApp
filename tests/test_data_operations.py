import pytest
from services.data_operations import (
    get_transactions,
...
class TestTransactions:
    ...
    def test_get_transaction_splits(self, setup_database):
        ...
        session.execute(
            text(
                "INSERT INTO transactions (transaction_date, amount, description) VALUES ('2023-01-01', 100.0, 'Test Transaction')"
            )
        )
        session.execute(
            text(
                "INSERT INTO transaction_splits (transaction_id, category_id, allocated_amount) VALUES (1, 1, 100.0)"
            )
        )
        session.commit()
...
class TestAccounts:
    ...
    def test_add_account(self, setup_database):
        ...
        another_result = add_account(
            another_unique_name, "Savings", "Another Bank", another_unique_identifier
        )
    ...
    def test_add_duplicate_account(self, setup_database):
        ...
        with pytest.raises(
            AccountAlreadyExistsError,
            match=f"An account with the name '{unique_name}' or bank identifier '{unique_identifier}' already exists",
        ):
            add_account(unique_name, "Savings", "Another Bank", unique_identifier)
        ...
        with pytest.raises(
            AccountAlreadyExistsError,
            match=f"An account with the name '{unique_name}' or bank identifier '.*' already exists",
        ):
            add_account(unique_name, "Savings", "Another Bank", f"bank_{uuid.uuid4().hex[:8]}")
        ...
        with pytest.raises(
            AccountAlreadyExistsError,
            match=f"An account with the name '.*' or bank identifier '{unique_identifier}' already exists",
        ):
            add_account(
                f"Different Account {uuid.uuid4().hex[:8]}",
                "Savings",
                "Another Bank",
                unique_identifier,
            )
