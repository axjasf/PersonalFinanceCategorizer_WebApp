...
new_bank_identifier = st.text_input(
    "Bank Identifier", value=suggested_identifier, key="new_account_bank_identifier"
)
...
if not accounts.empty:
    edit_account_id = st.selectbox(
        "Select Account to Edit", accounts["id"].tolist(), key="edit_account_select"
    )
    ...
    edit_institution = st.text_input(
        "Financial Institution", value=edit_account["institution"], key="edit_account_institution"
    )

    if st.button("Update Account"):
        success, message, _ = modify_account(
            edit_account_id, edit_name, edit_type, edit_institution
        )
        if success:
            st.success(message)
            st.rerun()
...
if not accounts.empty:
    delete_account_id = st.selectbox(
        "Select Account to Delete", accounts["id"].tolist(), key="delete_account_select"
    )
...
st.warning(
    "Note: Deleting an account will also delete all associated transactions and splits. This action cannot be undone."
)
